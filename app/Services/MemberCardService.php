...
class MemberCardService
{
    ...
    private function text(\GdImage $img, string $txt, int $x, int $y, int $sz, string $font, array $rgb, string $align = 'left'): void
    {
        if (!$font || !$txt) return;
        $c = $this->col($img, $rgb);
        if ($align === 'right')  $x -= $this->ttfw($sz, $font, $txt);
        if ($align === 'center') $x -= intdiv($this->ttfw($sz, $font, $txt), 2);
    ...
    private function insertCircularPhoto(\GdImage $card, string $path, int $cx, int $cy, int $r): void
    {
        ...
        $ox = $cx - $r; $oy = $cy - $r; $r2 = $r * $r;
        for ($x = 0; $x < $size; $x++) {
            for ($y = 0; $y < $size; $y++) {
                $dx = $x - $r; $dy = $y - $r;
                if ($dx * $dx + $dy * $dy <= $r2) {
                    $px = imagecolorat($sq, $x, $y);
                    $c  = imagecolorallocate($card, ($px >> 16) & 0xFF, ($px >> 8) & 0xFF, $px & 0xFF);
                    imagesetpixel($card, $ox + $x, $oy + $y, $c);
                }
            }
        }
        imagedestroy($sq);
    ...
    private function drawMemberInfo(\GdImage $img, Member $member, array $d): void
    {
        ...
        if (mb_strlen($name) > 22) $name = mb_substr($name, 0, 21) . '…';
        ...
        if (mb_strlen($prof) > 32) $prof = mb_substr($prof, 0, 31) . '…';
        if ($sans && $prof) {
    ...
    private function drawFooter(\GdImage $img, Member $member, array $d): void
    {
        ...
        $this->text($img, $since, $this->w - 36, $ty, 10, $sans, $d['since_c'], 'right');
    }
}
